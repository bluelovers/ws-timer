/**
 * Regression test: start(-1) on an empty queue must not corrupt fake_now.
 *
 * 迴歸測試：在空佇列上呼叫 start(-1) 不得破壞 fake_now。
 *
 * Background / 背景：
...
describe('issue: start(-1) on empty queue must not corrupt fake_now', () =>
{
	it('should keep fake_now valid after start(-1) on empty queue', async () =>
	{
		const t = new Timer();
		const before = t.timer.now();

		assert.ok(before.isValid(), 'precondition: initial fake_now is valid');

		await t.start(-1);

		// fake_now 不應變成無效日期
		// fake_now must not become an invalid date
		assert.ok(t.timer.now().isValid(), 'fake_now is still valid after start(-1) on empty queue');
	...
	it('should keep fake_now valid after start() with any negative amount on empty queue', async () =>
	{
		const t = new Timer();
