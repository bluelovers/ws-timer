 ...
 *   - 要取得「自建立以來經過的毫秒數」，用 `timer.now().diff(timer.data.fake_init)`。
 *     To get "elapsed fake ms since creation", use `timer.now().diff(timer.data.fake_init)`.
...

{
	...
	// 經過的毫秒 = now().diff(fake_init)
	// elapsed ms = now().diff(fake_init)
	check('初始 elapsed = 0', t.timer.now().diff(t.timer.data.fake_init) === 0);

	t.advance(1200);
	check('advance(1200) 後 elapsed = 1200', t.timer.now().diff(t.timer.data.fake_init) === 1200);
